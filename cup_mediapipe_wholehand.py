    ...
    def run(self):
        while self.running:
            ...
            if results.hand_landmarks:
                ...
                # --- 2. [전체 손 기준] 핵심 랜드마크 추출 ---
                # 엄지/검지 끝부분(오차가 심함) 대신, 손바닥을 구성하는 뼈대 중 가장 단단하고 형태가 변하지 않는 
                # '검지 손가락 뿌리 관절(5번)'과 '새끼 손가락 뿌리 관절(17번)'을 기준으로 삼습니다.
                # 이 두 점을 연결한 선은 손등의 평면 각도 전체를 대변합니다.
                index_mcp = self.smoothed_landmarks[5]
                pinky_mcp = self.smoothed_landmarks[17]

                # 화면에 그리기 위한 2D 픽셀 좌표
                index_mcp_x, index_mcp_y = int(index_mcp[0] * w), int(index_mcp[1] * h)
                pinky_mcp_x, pinky_mcp_y = int(pinky_mcp[0] * w), int(pinky_mcp[1] * h)

                # 기준 뼈대 연결하는 굵은 선 그리기 (시각적 확인용)
                cv2.line(frame, (index_mcp_x, index_mcp_y), (pinky_mcp_x, pinky_mcp_y), (255, 0, 255), 4)
                cv2.circle(frame, (index_mcp_x, index_mcp_y), 8, (0, 255, 255), -1)
                cv2.circle(frame, (pinky_mcp_x, pinky_mcp_y), 8, (0, 255, 255), -1)

                # --- 3. 3D 공간 각도(Yaw) 계산 ---
                dx_3d = index_mcp[0] - pinky_mcp[0] # 좌우
                dz_3d = index_mcp[2] - pinky_mcp[2] # 앞뒤(깊이)
...
def main():
    ...
    print("['s' 키]를 누르면 두 카메라에서 보이는 현재 손바닥 각도를 0도(기준점)로 설정합니다.")
    ...
    while True:
        ...
        if display_frame is not None:
            ...
            cv2.imshow('Dual Camera Whole Hand Rotation Tracker', display_frame)
