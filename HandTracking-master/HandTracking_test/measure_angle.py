...
def estimate_rotation(initial_points, current_points):
    """SVD를 이용한 3D 회전 행렬 계산"""
    centroid_init = np.mean(initial_points, axis=0) # 세 손가락에 대한 초기 원점
    centroid_curr = np.mean(current_points, axis=0) # 세 손가락에 대한 현재 원점
    A = initial_points - centroid_init # 0도에 대한 포인트 정보
    B = current_points - centroid_curr # 현재 포인트 정보
    H = A.T @ B # 상관관계 행렬 -> 어느 방향으로 얼마나 옮겨갔는가에 대한 값
    U, S, Vt = np.linalg.svd(H) #SVD 알고리즘 적용 -> 공간이 꺽인 방향 및 회전에 대한 정보 추출
    R = Vt.T @ U.T # 회전행렬
    if np.linalg.det(R) < 0: # SVD 알고리즘 거울 반사 방지 -> 오차를 줄이는 과정에서 가장 빠른, 쉬운 방법으로 하기 때문에 가끔 Z값을 변환시켜 최소값을 구함. 이를 방지하기 위함임.
        Vt[-1, :] *= -1
        R = Vt.T @ U.T
    return R
...
def draw_info_panel(img, angles, total_angle, is_grabbing):
    ...
    status_text = "GRABBING VIRTUAL OBJ" if is_grabbing else "IDLE"
...
last_angles = None
last_total_angle = None

# EMA 스무딩 설정
SMOOTH_ALPHA = 0.4  # 낮을수록 더 부드러움 (0.2~0.6 조절)
smoothed_points_3d = None
smoothed_points_2d = None
...
while cap.isOpened():
    ...
    if results.hand_world_landmarks and results.hand_landmarks:
        ...
        # EMA 스무딩 적용
        if smoothed_points_3d is None:
            smoothed_points_3d = current_points_3d.copy()
            smoothed_points_2d = np.array(points_2d, dtype=float)
        else:
            smoothed_points_3d = SMOOTH_ALPHA * current_points_3d + (1 - SMOOTH_ALPHA) * smoothed_points_3d
            smoothed_points_2d = SMOOTH_ALPHA * np.array(points_2d) + (1 - SMOOTH_ALPHA) * smoothed_points_2d

        current_points_3d = smoothed_points_3d
        points_2d = [(int(p[0]), int(p[1])) for p in smoothed_points_2d]

        # 세 손가락 끝 점 시각화
        for pt in points_2d:
            cv2.circle(image, pt, 6, (255, 0, 255), -1)
        ...
        if grabbed_points_3d is not None:
            # 회전 행렬 및 각도 계산
            R = estimate_rotation(grabbed_points_3d, current_points_3d)
        ...
        # 손을 잠깐 놓쳤을 때 마지막 각도 유지
        draw_info_panel(image, last_angles, last_total_angle, is_grabbing=(last_angles is not None))

    cv2.imshow('Virtual Object Rotation Tracker', image)

    key = cv2.waitKey(1) & 0xFF
    if key == 27:
        break
    elif key == ord('g'):
        if 'current_points_3d' in locals():
