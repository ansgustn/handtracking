...
def main():
    ...
    print("[INFO] YOLOv11-Pose 모델 로드 중...")
    # YOLO11-Pose 모델 로드 (yolo11n-pose.pt가 없으면 자동 다운로드됨)
    # yolo11n-pose.pt는 사람의 신체 주요 관절 17개를 추출합니다. (손목, 팔꿈치, 어깨 등 포함)
    yolo_model = YOLO('yolo11n-pose.pt') 

    print("[INFO] YOLO11-Pose 다중 카메라 실험 시작! 추출되는 Keypoints(관절)를 확인합니다. 종료: 'q'")
        ...
        while True:
            ...
            if webcam_available:
                ...
                if ret:
                    ...
                    cv2.putText(web_res_frame, 'WebCam (YOLO11-Pose)', (20, 40), cv2.FONT_HERSHEY_DUPLEX, 1, (255, 0, 0), 2)
            ...
            if kinect_available:
                ...
                if ret_color:
                    ...
                    cv2.putText(kinect_res_frame, 'Kinect (YOLO11-Pose)', (20, 40), cv2.FONT_HERSHEY_DUPLEX, 1, (0, 0, 255), 2)
            ...
            cv2.imshow('YOLO11-Pose Multi-Cam Test', display_img)
